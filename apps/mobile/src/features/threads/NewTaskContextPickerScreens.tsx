...
export function NewTaskEnvironmentPickerRouteScreen() {
  ...
  const blockedForGenericChat = isGenericChatProject(flow.selectedProject);

  useEffect(() => {
    if (blockedForGenericChat) navigation.goBack();
  }, [blockedForGenericChat, navigation]);

  if (blockedForGenericChat) {
    return <View className="flex-1 bg-sheet" collapsable={false} />;
  }
