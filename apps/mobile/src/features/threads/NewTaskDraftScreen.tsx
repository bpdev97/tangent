...
export function NewTaskDraftScreen(props: {
  ...
  const composerWorkspaceCwd = isGenericChat
    ? null
  ...
  const composerMenu = useComposerCommandMenu({
    ...
    pullRequestProjectId:
      !isGenericChat && selectedEnvironmentServerConfig?.environment.capabilities.pullRequests
  ...
  const workspaceControls = isGenericChat ? null : (
  ...
  if (isAndroid) {
    return (
      <View className="flex-1 bg-sheet" collapsable={false}>
        <NativeStackScreenOptions options={{ headerShown: false }} />
        <AndroidScreenHeader
          title={isGenericChat ? "New chat" : "New task"}
          onBack={closeNewTask}
        />
        {heroViewport}
