...
describe("mobile image preparation", () => {
  ...
  it("converts small HEIC images to JPEG so uploads use a provider-readable type", async () => {
    expect(
      await prepareComposerImage({
        ...input,
        base64: "YWJj",
        mimeType: "image/heic",
        name: "a.HEIC",
      }),
    ).toMatchObject({ name: "a.jpg", mimeType: "image/jpeg", sizeBytes: 4 });
    expect(native.save.mock.calls[0]?.[0]).toMatchObject({ format: "jpeg" });
  });
...
describe("mobile image entry points", () => {
  ...
  it("skips a failed native paste while retaining valid images and cleaning both sources", async () => {
