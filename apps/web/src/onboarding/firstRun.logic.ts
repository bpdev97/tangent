...
/** The server-managed Chats project is not evidence of prior user setup. */
export function getFirstRunProjects<T extends { readonly id: string }>(projects: ReadonlyArray<T>) {
  return projects.filter((project) => !isGenericChatProjectId(project.id));
}
