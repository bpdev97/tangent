...
describe("managed Chats onboarding", () => {
  const chatProject = {
    id: GENERIC_CHAT_PROJECT_ID,
    environmentId: primaryEnvironmentId,
    workspaceRoot: "/scratch/generic-chat",
  };

  it.each([false, true])("shows onboarding with empty Chats and cwd bootstrap %s", (bootstrap) => {
    const projects = getFirstRunProjects([
      chatProject,
      { ...chatProject, environmentId: "remote-environment" },
      ...(bootstrap ? [bootstrapProject] : []),
    ]);
    const threads = bootstrap ? [bootstrapThread] : [];
    const workspaceFresh = isFreshFirstRunWorkspace({
      primaryEnvironmentId,
      serverCwd: bootstrapProject.workspaceRoot,
      bootstrapProjectId: bootstrapProject.id,
      bootstrapThreadId: bootstrapThread.id,
      bootstrapProjectCreated: bootstrap,
      bootstrapThreadCreated: bootstrap,
      projects,
      threads,
    });
  ...
  it("recognizes an existing chat even without an ordinary project", () => {
    const projects = getFirstRunProjects([chatProject]);
    const threads = [
      {
        ...bootstrapThread,
        projectId: GENERIC_CHAT_PROJECT_ID,
        latestUserMessageAt: "2026-09-05T12:00:00.000Z",
      },
    ];
    const workspaceFresh = isFreshFirstRunWorkspace({
      primaryEnvironmentId,
