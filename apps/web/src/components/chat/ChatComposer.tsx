  ...
  const addComposerImages = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (!isProviderSendTurnSupportedImageMimeType(file.type)) {
        error = `'${file.name}' is not a supported image type. Attach GIF, HEIC, JPEG, PNG, or WebP images.`;
        continue;
      }
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-0.5">
                <ComposerPendingApprovalActions
                  ...
                  supportsSessionPersistence={Boolean(
                    activePendingApproval.supportsSessionPersistence,
                  )}
                ...
                <div className="-m-1 -ms-3.5 flex min-w-0 flex-1 items-center gap-1 overflow-x-auto p-1 ps-3.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  ...
                  {isComposerFooterCompact ? (
                    <CompactComposerControlsMenu
                      ...
                      runtimeMode={runtimeMode}
                      showRuntimeModeControl={!runtimeModeLocked}
