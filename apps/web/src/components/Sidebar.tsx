...
import { LinearPrBadge } from "./linear/LinearPrBadge";
          ...
          {projectTitle ? (
            <div className="flex min-w-0 items-center gap-2">
              <ProjectFavicon
                ...
                {...(isGenericChatProjectId(thread.projectId)
...
export default function Sidebar() {
    ...
    () =>
      new Map(
        projects.map((project) => [
          ...
          isGenericChatProjectId(project.id) ? null : project.workspaceRoot,
        ]),
      ),
    [projects],
  );
  ...
  const projectGroupByScopeKey = useMemo(
    () => new Map(regularProjectGroups.map((project) => [project.projectKey, project] as const)),
    [regularProjectGroups],
  );
  ...
  const scopedProjectIsGenericChat =
    scopedProjectGroup !== null && isGenericChatProjectId(scopedProjectGroup.id);
  ...
  const openProjectSettings = useCallback(
    (projectGroup: SidebarProjectSnapshot) => {
      if (isGenericChatProjectId(projectGroup.id)) return;
      if (isMobile) {
        setOpenMobile(false);
      }
  ...
  const handleNewThreadClick = useCallback(
    (event?: ReactMouseEvent) => {
      // Generic chats are a separate surface and never count as project
      // destinations. Shift+click creates directly in the current regular
      // project when several project destinations exist.
      if (
        shouldCreateNewThreadInCurrentProject(event?.shiftKey ?? false, regularProjectGroups.length)
      ) {
                  ...
                  <TooltipTrigger
                    render={
                      <SidebarMenuButton
                        ...
                        disabled={regularProjectGroups.length === 0}
