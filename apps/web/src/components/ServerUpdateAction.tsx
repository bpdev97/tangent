...
/**
 * The package install on the server side is capped at 10 minutes; expire the
 ...
 * (running another CLI there would start a second server, not update this one),
    ...
    void Promise.resolve()
      ...
      .then((result) => {
        ...
        toastManager.add({
          ...
          description: `Tangent ${result.value.targetVersion} is installed — the server is restarting and will reconnect shortly.`,
        });
      })
      .catch((error: unknown) => {
        if (!ownsAttempt()) return;
