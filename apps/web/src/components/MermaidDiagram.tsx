...
function MermaidSource(props: { readonly code: string; readonly failed: boolean }) {
  return (
    <>
      {props.failed ? (
        <div data-chat-markdown-mermaid-error role="alert">
...
function MermaidViewport(props: {
  ...
  return (
    <div
      data-chat-markdown-mermaid-viewport
    ...
    >
      <div
        data-chat-markdown-mermaid-svg
        style={
          props.maximized
...
function MermaidPresentation({
  ...
  const sourceVisible = props.showSource || result === null;
  const maximizeLabel = props.maximized ? "Restore diagram" : "Maximize diagram";

  return (
    <>
      <div data-chat-markdown-mermaid-header>
        <span data-chat-markdown-mermaid-title>Mermaid</span>
        <span data-chat-markdown-mermaid-actions role="toolbar" aria-label="Diagram actions">
          {!sourceVisible ? (
            <>
              <Button
                variant="ghost-muted"
              ...
              <span data-chat-markdown-mermaid-scale>{Math.round(props.scale * 100)}%</span>
              <Button
                variant="ghost-muted"
                size="icon-xs"
...
export const MermaidDiagram = memo(function MermaidDiagram(props: {
  ...
  // Keyed so zoom, source, and maximized state reset when the diagram or theme changes.
  return <MermaidDiagramView key={`${props.theme}\0${props.code}`} {...props} />;
});

function MermaidDiagramView(props: { readonly code: string; readonly theme: MermaidTheme }) {
  const reactId = useId();
  ...
  return (
    <>
      ...
      {maximized && result !== null ? (
        ...
        >
          <DialogPopup
            className="h-[calc(100dvh-2rem)] max-h-none w-[calc(100vw-2rem)] max-w-none overflow-hidden"
          ...
          >
            ...
            {/* The Mermaid styles are scoped under .chat-markdown; the dialog is portaled out of it. */}
            <div className="chat-markdown contents">
              <div data-chat-markdown-mermaid data-chat-markdown-mermaid-fullscreen>
                <MermaidPresentation
                  code={props.code}
                  result={result}
                  failed={false}
