...
export function configFromSettings(
  ...
  // Settings arrive materialized, so a saved password is the real value here.
  const saved = settings.personalPushRelay;
  if (!saved.url && !saved.password) return fallback;
  return {
...
/**
 * Reads whether a personal relay is configured. Settings are optional, so a
 * layer built without them still honors the environment configuration.
 */
export const makeConfiguredReader = Effect.gen(function* () {
  const config = yield* ServerConfig.ServerConfig;
  const settings = yield* Effect.serviceOption(ServerSettingsModule.ServerSettingsService);
  return yield* Effect.succeed(
    Option.match(settings, {
      onNone: () => Effect.succeed(make(configFromServerConfig(config)).configured),
      onSome: (service) =>
        makeFromRuntime(config, service).pipe(Effect.map((client) => client.configured)),
    }),
  );
});

/** Authenticated snapshot read used by the settings "Test connection" action. */
export const testConnection = (
  config: ServerConfig.ServerConfig["Service"],
  settingsService: ServerSettingsModule.ServerSettingsService["Service"],
): Effect.Effect<PersonalPushRelayTestResult> =>
  makeFromRuntime(config, settingsService).pipe(
