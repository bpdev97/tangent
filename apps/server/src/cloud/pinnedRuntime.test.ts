import { cliRuntimeInstallSentinel } from "@t3tools/shared/cliRelease";
...
const version = "1.2.3";
const archiveName = `tangent-server-${version}-linux-x64.tar.gz`;
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs the verified release archive as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(requests, [
        `https://releases.example/download/personal-v${version}/SHA256SUMS`,
        `https://releases.example/download/personal-v${version}/${archiveName}`,
      ]);
      assert.deepEqual(commands, ["tar"]);
      assert.equal(
        yield* fs.readFileString(paths.sentinelPath),
        cliRuntimeInstallSentinel(version, "https://releases.example/download"),
      );
  ...
  it.effect.each([true, false])(
    ...
    (knownSize) =>
      Effect.gen(function* () {
        ...
        assert.equal(
          yield* fs.readFileString(installed.sentinelPath),
          cliRuntimeInstallSentinel(version),
        );
      }),
  );
  ...
  it.effect("replaces a same-version runtime from another release source", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "tangent-runtime-source-" });
      const paths = pinnedRuntimePaths(path, baseDir, version, "linux");
      yield* fs.makeDirectory(paths.versionDir, { recursive: true });
      yield* fs.writeFileString(paths.entryPath, "upstream binary");
      yield* fs.writeFileString(
        paths.sentinelPath,
        `${version}\nhttps://github.com/pingdotgg/t3code/releases/download/v${version}\n`,
      );
      const requests: string[] = [];
      ...
      assert.notEqual(yield* fs.readFileString(paths.entryPath), "upstream binary");
    }),
  );

  it.effect.each(["archive length", "chunked checksums"])("bounds downloads (%s)", (mode) =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "tangent-runtime-limit-" });
      const checksums = yield* validChecksums;
      const client = HttpClient.make((request) =>
        Effect.succeed(
          HttpClientResponse.fromWeb(
            request,
            request.url.endsWith("/SHA256SUMS")
              ? new Response(mode === "chunked checksums" ? "x".repeat(64 * 1024 + 1) : checksums)
      ...
      const error = yield* ensurePinnedRuntimeInstalled({
        ...
        httpClient: client,
        runner: extractingRunner(fs, path, commands),
        validate: () => Effect.die("must not validate oversized assets"),
      }).pipe(Effect.flip);
      assert.include(error.message, "exceeds");
      assert.deepEqual(commands, []);
      assert.deepEqual(yield* fs.readDirectory(path.join(baseDir, "runtime", "versions")), []);
      assert.isFalse(yield* fs.exists(path.join(baseDir, "runtime", "archives")));
    }),
  );

  it.effect("retains the current and one prior verified archive", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "tangent-runtime-retention-" });
      const hash = yield* archiveHex(archiveBytes);
      for (const releaseVersion of ["1.2.1", "1.2.2", "1.2.3"]) {
        yield* ensurePinnedRuntimeInstalled({
          baseDir,
          ...
          httpClient: releaseHttpClient(
            `${hash}  tangent-server-${releaseVersion}-linux-x64.tar.gz\n`,
          ),
          runner: extractingRunner(fs, path),
          validate: () => Effect.void,
        });
      }
      const archivesDir = path.join(baseDir, "runtime", "archives");
      assert.deepEqual((yield* fs.readDirectory(archivesDir)).sort(), [
        "tangent-server-1.2.2-linux-x64.tar.gz",
        "tangent-server-1.2.3-linux-x64.tar.gz",
      ]);
      assert.deepEqual(yield* fs.readFile(path.join(archivesDir, archiveName)), archiveBytes);
    }),
  );
  ...
  it.effect("preserves a completed runtime when validation fails", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(finalPaths.sentinelPath, cliRuntimeInstallSentinel(version));
