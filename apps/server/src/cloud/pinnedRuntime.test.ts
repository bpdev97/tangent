...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("serializes concurrent installs of the same runtime", () =>
    Effect.gen(function* () {
      ...
      const install = ensurePinnedRuntimeInstalled({
        baseDir,
        version: "0.0.29",
        packageSpecifier: "/tmp/tangent-server-0.0.29.tgz",
        installIdentity: "test:0.0.29",
        fs,
        path,
        runner,
      });
    ...
    Effect.gen(function* () {
      ...
      const installFiber = yield* Effect.forkChild(
        ensurePinnedRuntimeInstalled({
          baseDir,
          version: "0.0.30",
          packageSpecifier: "/tmp/tangent-server-0.0.30.tgz",
          installIdentity: "test:0.0.30",
          fs,
          path,
          runner,
        }),
  ...
  it.effect("reinstalls when the source identity changes for the same version", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-pinned-runtime-test-" });
      const paths = pinnedRuntimePaths(path, baseDir, "0.0.31");
      const installedSpecifiers: string[] = [];
      const runner = ProcessRunner.ProcessRunner.of({
        run: (input) =>
          Effect.gen(function* () {
            installedSpecifiers.push(input.args.at(-1) ?? "");
            yield* fs
              .makeDirectory(path.dirname(paths.entryPath), { recursive: true })
              .pipe(Effect.orDie);
            yield* fs.writeFileString(paths.entryPath, "export {};\n").pipe(Effect.orDie);
      ...
      for (const source of [
        {
          packageSpecifier: "/tmp/upstream-package.tgz",
          installIdentity: "npm:t3@0.0.31",
        },
        {
          packageSpecifier: "/tmp/tangent-server-0.0.31.tgz",
          installIdentity: "github-release:personal-v0.0.31:abc123",
        },
      ]) {
      ...
      assert.deepEqual(installedSpecifiers, [
        "/tmp/upstream-package.tgz",
        "/tmp/tangent-server-0.0.31.tgz",
      ]);
      assert.equal(
        yield* fs.readFileString(paths.sentinelPath),
        "github-release:personal-v0.0.31:abc123\n",
      );
    }),
  );
});
