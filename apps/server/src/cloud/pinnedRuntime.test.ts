...
import { encodeRuntimeInstallSentinel } from "./serviceProtocol.ts";

const packageSpecifier = "/tmp/tangent-server-1.2.3.tgz";
const installIdentity = `github-release:v1.2.3:${"a".repeat(64)}`;
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs through pnpm when its Node runtime has no npm executable", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        ...
        encodeRuntimeInstallSentinel("1.2.3", installIdentity),
      );
      assert.isTrue(commands[0]!.args.includes(packageSpecifier));
    }),
  );
  ...
  it.effect("replaces a completed runtime from a different distribution source", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-pinned-runtime-source-" });
      const finalPaths = pinnedRuntimePaths(path, baseDir, "1.2.3");
      yield* fs.makeDirectory(path.dirname(finalPaths.entryPath), { recursive: true });
      yield* fs.writeFileString(finalPaths.entryPath, "upstream package\n");
      yield* fs.writeFileString(
        finalPaths.sentinelPath,
        encodeRuntimeInstallSentinel("1.2.3", "npm:t3@1.2.3"),
      );
