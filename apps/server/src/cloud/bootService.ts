...
import { CLI_RELEASE_BASE_URL_ENV, cliRuntimeInstallSentinel } from "@t3tools/shared/cliRelease";
...
const DEFAULT_BOOT_SERVICE_IDENTITY = {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    "[Unit]",
    `Description=${plan.displayName ?? DEFAULT_BOOT_SERVICE_IDENTITY.displayName} server`,
...
export function renderBootServicePlist(
  ...
  return [
    ...
    `<dict>`,
    `  <key>Label</key>`,
    `  <string>${plan.launchdLabel ?? BOOT_SERVICE_LAUNCHD_LABEL}</string>`,
    ...
    `    <string>${plan.plistFile ?? BOOT_SERVICE_PLIST_FILE}</string>`,
    `  </dict>`,
...
function systemdManager(input: {
  ...
  const unitFile = bootServiceUnitFile(input.identity ?? DEFAULT_BOOT_SERVICE_IDENTITY);
  ...
  return {
    ...
    render: (plan) =>
      renderBootServicePlist(
        { ...plan, launchdLabel: identity.launchdLabel, plistFile },
        { homeDir: input.homeDir, environmentPath: input.environmentPath },
      ),
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the background service.";
  }
}
...
export function formatBootServiceProblem(
  ...
  switch (problem) {
    ...
    case "linger-disabled":
      return `Lingering is disabled. ${identity.displayName} will stop when your last login session ends and will not start at boot. Run \`sudo loginctl enable-linger "$(id -un)"\` on this machine, then retry the service command as your normal user.`;
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `t3 service install` to repair it.";
    case "service-stopped":
      return `The service is not running. Check the service log and \`systemctl --user status ${bootServiceUnitFile(identity)}\`, then run \`t3 service install\`.`;
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const expectedRuntimeSentinel = cliRuntimeInstallSentinel(input.cliVersion, releaseBaseUrl);
  ...
  const plan: BootServicePlan = {
    ...
    plistFile: bootServicePlistFile(identity),
  };
  ...
  const readSystemdProblems = Effect.fn("cloud.boot_service.read_systemd_problems")(function* (
    ...
    if (includeService && succeeded(manager)) {
      const [enabled, active] = yield* Effect.all(
        [
          probe("systemctl", ["--user", "is-enabled", unitFile]),
          probe("systemctl", ["--user", "is-active", unitFile]),
        ],
        { concurrency: "unbounded" },
      );
      if (
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned server runtime",
                  cause,
                }),
            ),
  ...
  const status: BootService["Service"]["status"] = Effect.gen(function* () {
    ...
    return {
      ...
      current:
        ...
        runtimeSentinel.value === expectedRuntimeSentinel &&
