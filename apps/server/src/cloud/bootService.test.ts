...
it("renders the fork-owned Tangent service identity", () => {
  const unit = BootService.renderBootServiceUnit({
    nodePath: "/usr/bin/node",
    launcherPath: "/home/user/.bpdev-code/runtime/service-launcher.mjs",
    baseDir: "/home/user/.bpdev-code",
    logPath: "/home/user/.bpdev-code/userdata/logs/boot-service.log",
    unitPath: "/home/user/.config/systemd/user/tangent.service",
    displayName: "Tangent",
    unitFile: "tangent.service",
  });

  expect(unit).toContain("Description=Tangent server");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  options: {
    ...
    readonly usePinnedLauncher?: boolean;
    readonly installerPath?: string;
  } = {},
) {
  const platform = options.platform ?? "linux";
  const identity = options.identity ?? {
    bootServiceName: "t3code",
  ...
  yield* fs.writeFileString(
    runtime.sentinelPath,
    encodeRuntimeInstallSentinel("1.2.3", `current-cli:${identity.bootServiceName}:1.2.3`),
  );
  ...
  const makeService = (environmentPath = options.installerPath ?? macInstallerPath) =>
    BootService.make({
      baseDir,
      ...
      host: {
        ...
        cliEntryPath: path.join(home, "node_modules", "t3", "dist", "bin.mjs"),
        ...(options.usePinnedLauncher ? {} : { launcherSourcePath: sourceLauncher }),
      },
      identity,
    }).pipe(
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("copies the launcher from the prepared pinned runtime", () =>
    Effect.gen(function* () {
      const { service, fs } = yield* makeHarness({ usePinnedLauncher: true });
      const plan = yield* service.install;
  ...
  it.effect("installs and controls the Tangent unit name", () =>
    Effect.gen(function* () {
      const { service, fs, commands } = yield* makeHarness({
        identity: {
      ...
      expect(plan.unitPath).toMatch(/tangent\.service$/);
      expect(yield* fs.readFileString(plan.unitPath)).toContain("Description=Tangent server");
      expect(commands).toContain("systemctl --user enable tangent.service");
      expect(commands).toContain("systemctl --user restart tangent.service");
    }),
  );
  ...
  it.effect("fails closed on Windows", () =>
    Effect.gen(function* () {
      const { service } = yield* makeHarness({ platform: "win32" });
  ...
  it.effect("installs and controls the Tangent launch agent identity", () =>
    Effect.gen(function* () {
      const { service, fs, commands } = yield* makeHarness({
        platform: "darwin",
        identity: {
      ...
      expect(plan.unitPath).toMatch(/com\.bpdev97\.tangent\.service\.plist$/);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
        "<string>com.bpdev97.tangent.service</string>",
      );
      expect(commands).toContain("launchctl enable gui/501/com.bpdev97.tangent.service");
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      const { service, commands, control } = yield* makeHarness({ platform: "darwin" });
      yield* service.install;
  ...
  it.effect("reconstructs a launch agent search path when the installer has no PATH", () =>
    Effect.gen(function* () {
      const { service, fs } = yield* makeHarness({ platform: "darwin", installerPath: "" });
      const plan = yield* service.install;
  ...
  it.effect("keeps an installed launch agent current when the process PATH changes", () =>
    Effect.gen(function* () {
      const { service, makeService } = yield* makeHarness({ platform: "darwin" });
      yield* service.install;
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      const { service, control } = yield* makeHarness({ platform: "darwin" });
      yield* service.install;
  ...
  it.effect("restarts without overwriting a pending remote update on macOS", () =>
    Effect.gen(function* () {
      const { service, fs, statePath, commands } = yield* makeHarness({ platform: "darwin" });
      yield* service.install;
      const plistPath = (yield* service.status).unitPath;
      // @effect-diagnostics-next-line preferSchemaOverJson:off - fixed launcher-owned test document.
      const pendingState = JSON.stringify({
