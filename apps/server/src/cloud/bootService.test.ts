...
it.layer(NodeServices.layer)("BootService", (it) => {
  ...
  it.effect("uses a downstream-specific service identity without colliding with T3 Code", () =>
    Effect.gen(function* () {
      const { dirs, fs, path } = yield* makeTestContext();
      const commands: Array<RecordedCommand> = [];
      const service = yield* BootService.make({
        ...
        cliVersion: "0.0.27",
        host: makeHost(dirs.stableEntry),
        identity: { bootServiceName: "tangent", displayName: "Tangent" },
      }).pipe(Effect.provide(makeRecordingRunnerLayer(commands)), provideHostRefs(dirs.home));

      const plan = yield* service.install;
      const unitPath = path.join(dirs.home, ".config", "systemd", "user", "tangent.service");
      assert.equal(plan.unitPath, unitPath);
      assert.include(yield* fs.readFileString(unitPath), "Description=Tangent server (T3 Connect)");
      assert.deepEqual(
        commands
          .filter(
            (entry) => entry.command === "systemctl" && entry.args.includes("tangent.service"),
          )
          .map((entry) => entry.args.join(" ")),
        ["--user enable tangent.service", "--user restart tangent.service"],
      );
      assert.isFalse(
  ...
  it.effect("pins a runtime from the current package when running from the npx cache", () =>
    Effect.gen(function* () {
      const { dirs, fs, path } = yield* makeTestContext();
      const commands: Array<RecordedCommand> = [];
      const service = yield* BootService.make({
      ...
      assert.deepEqual(commands[0], {
        ...
        args: [
          ...
          "--no-fund",
          "--no-audit",
          "/home/theo/.npm/_npx/abc/node_modules/t3",
        ],
      });
