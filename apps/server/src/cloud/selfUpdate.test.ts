...
import { BOOT_SERVICE_UNIT_ENV, renderBootServiceUnit } from "./bootService.ts";
...
it("recognizes installed package artifacts as swappable entry points", () => {
...
it.layer(NodeServices.layer)("resolveServerSelfUpdateCapability", (it) => {
  ...
  const writeUnitReferencing = Effect.fn("test.writeUnitReferencing")(function* (
    ...
    yield* fs.writeFileString(
      path.join(unitDir, SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE),
      renderBootServiceUnit({
        ...
        unitPath: path.join(unitDir, SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE),
        displayName: "Tangent",
        unitFile: SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE,
      }),
    );
  });
      ...
      const method = yield* SelfUpdate.resolveServerSelfUpdateCapability({
        ...
        provideHostRefs({
          ...
          env: {
            ...
            [BOOT_SERVICE_UNIT_ENV]: SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE,
          },
          entryPath,
        }),
      );
...
it.layer(NodeServices.layer)("ServerSelfUpdate.update", (it) => {
  ...
  const makeContext = Effect.fn("test.makeContext")(function* (options?: {
    ...
    const service = yield* SelfUpdate.make({
      downloadRelease: ({ baseDir: releaseBaseDir, version }) =>
        Effect.succeed({
          packagePath: path.join(
            releaseBaseDir,
            "runtime",
            "downloads",
            `tangent-server-${version}.tgz`,
          ),
          sha256: "test-checksum",
          installIdentity: `test:${version}`,
        }),
      host: {
  ...
  it.effect("rejects dist-tags and other non-exact versions", () =>
    Effect.gen(function* () {
      ...
      assert.include(error.reason, "not an exact Tangent version");
  ...
  it.effect("surfaces a failed package install and never schedules a restart", () =>
    Effect.gen(function* () {
      const context = yield* makeContext({ failWhen: (command) => command === "npm" });
      const error = yield* context.service.update({ targetVersion: "0.0.29" }).pipe(Effect.flip);
      assert.equal(error.reason, "Could not install the requested Tangent version.");
  ...
  it.effect("installs, preflights, and respawns a foreground server", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        [
          `npm install --prefix ${context.path.join(context.baseDir, "runtime/versions/0.0.29")} --no-fund --no-audit ${context.path.join(context.baseDir, "runtime/downloads/tangent-server-0.0.29.tgz")}`,
    ...
    Effect.gen(function* () {
      ...
      const unit = yield* context.fs.readFileString(
        context.path.join(
          ...
          "systemd",
          "user",
          SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE,
        ),
      );
      ...
      assert.deepEqual(context.commands[3], {
        command: "systemctl",
        args: ["--user", "restart", SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE],
      });
      assert.lengthOf(context.spawns, 0);
  ...
  it.effect("restores the previous unit and permits a retry when systemd restart fails", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        [
          ["--user", "restart", SelfUpdate.PERSONAL_BOOT_SERVICE_UNIT_FILE],
          ["--user", "daemon-reload"],
        ],
      );
