  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      ...
      return yield* failWith(
        `This server is managed by the ${PERSONAL_DISTRIBUTION.connect.displayName} desktop app on its machine; update the desktop app to update it.`,
      );
    }
    if (capability === null) {
      return yield* failWith(
        `Remote updates require the ${PERSONAL_DISTRIBUTION.connect.displayName} background service. Run \`t3 service install\` on the server machine.`,
      );
    }
