...
async function runtimeExists(baseDir: string, version: string): Promise<boolean> {
  ...
  try {
    ...
    return entry.isFile() && runtimeInstallSentinelVersion(sentinel) === version;
  } catch {
    return false;
  }
}
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected server ${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  ...
  if (baseDir === undefined || baseDir === "") {
    throw new Error("T3CODE_HOME is required by the server service launcher.");
  }
