...
export const make = (options?: StartupOptions) =>
  Effect.gen(function* () {
    ...
    const startup = Effect.gen(function* () {
      ...
      // Provisioning writes the orchestration database, so a trial runtime must
      // wait until the launcher has committed it before repairing this managed
      // project. Commands remain gated until the repair finishes.
      yield* runStartupPhase(
        "generic-chat.ensure",
