...
// Tangent(FORK-IMAGE-001): HEIC/HEIF uploads become JPEG once, at ingestion,
// so every provider and stored attachment sees the same widely supported file.
...
export class ImageNormalizationError extends Data.TaggedError("ImageNormalizationError")<{
  readonly code: NormalizationWorkerFailureCode;
}> {
  override get message(): string {
    return NORMALIZATION_ERROR_MESSAGES[this.code];
  }
}
...
function transcodeHeifToJpeg(
  ...
  return Effect.callback<Uint8Array, ImageNormalizationError>((resume) => {
    ...
    worker.once("message", (message: NormalizationWorkerMessage) => {
      settle(
        message.ok
          ...
          : Effect.fail(new ImageNormalizationError({ code: message.code })),
      );
    });
    worker.once("error", () => {
      settle(Effect.fail(new ImageNormalizationError({ code: "decode" })));
    });
    worker.once("exit", () => {
      settle(Effect.fail(new ImageNormalizationError({ code: "decode" })));
    });
    ...
    Effect.timeoutOrElse({
      ...
      orElse: () => Effect.fail(new ImageNormalizationError({ code: "timeout" })),
    }),
  );
}
...
export const normalizeUploadedImage = Effect.fn("imageNormalization.normalizeUploadedImage")(
  function* (input: {
    ...
    if (!HEIF_MIME_TYPES.has(mimeType) && !hasHeifSignature(input.bytes)) {
      // Everything else passes through untouched, bytes and metadata alike.
      return input satisfies NormalizedUploadedImage;
    }
