...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
  ...
  it.effect("reports personal-only agent activity publishing", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-server-environment-personal-publish-test-",
      });
      const layer = ServerEnvironment.layer.pipe(
        Layer.provide(ServerSecretStore.layer),
        Layer.provide(
      ...
      expect(descriptor.capabilities.agentActivityPublishing).toBe(true);
    }),
  );
