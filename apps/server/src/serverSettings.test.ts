...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("rolls back provider secret changes when the settings file commit fails", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* serverSettings.updateSettings({
          personalPushRelay: { url: "https://push.example.test", password: "push-kept" },
          providerInstances: {
            [instanceId]: {
              driver: ProviderDriverKind.make("codex"),
        ...
        const failedUpdate = yield* serverSettings
          .updateSettings({
            personalPushRelay: { password: "push-new", passwordRedacted: false },
            providerInstances: {
              [instanceId]: {
                driver: ProviderDriverKind.make("codex"),
        ...
        assert.equal((yield* serverSettings.getSettings).personalPushRelay.password, "push-kept");
        assert.equal(
        ...
        const failed = yield* serverSettings
          .updateSettings({
            providerInstances: {},
            personalPushRelay: { password: "", passwordRedacted: false },
          })
          .pipe(Effect.result);
  ...
  for (const secretTarget of ["provider", "personal push"] as const) {
    for (const failure of ["response materialization", "partially committed write"] as const) {
      it.effect(`rolls back settings secrets after ${secretTarget} ${failure} fails`, () => {
              ...
              Effect.suspend(() => {
                ...
                if (
                  ...
                  textDecoder.decode(value) ===
                    (secretTarget === "provider" ? "sk-new" : "push-new")
                ) {
                  return Effect.fail(
