...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription(`Install ${SERVICE_DISPLAY_NAME} as a background service for this user.`),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `${SERVICE_DISPLAY_NAME} service is already installed with version ${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `${SERVICE_DISPLAY_NAME} service is already using version ${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription(`Stop and remove the ${SERVICE_DISPLAY_NAME} background service.`),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed
            ? `Removed the ${SERVICE_DISPLAY_NAME} service.`
            : `${SERVICE_DISPLAY_NAME} service is not installed.`,
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription(
    `Show whether the ${SERVICE_DISPLAY_NAME} background service is installed.`,
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log(
      `${SERVICE_DISPLAY_NAME} is already set up to run in the background on this machine.`,
    );
    return true;
  }
  if (
    ...
    yield* Console.log(
      `A newer ${SERVICE_DISPLAY_NAME} ${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ...
        : platform === "darwin"
          ? `Run ${SERVICE_DISPLAY_NAME} in the background whenever you log in to this Mac? ` +
            "It stays reachable through T3 Connect while you are logged in."
          : `Run ${SERVICE_DISPLAY_NAME} in the background whenever this machine boots? ` +
