...
const preparePackageResource = Effect.fn("preparePackageResource")(function* (
  repoRoot: string,
  serverDir: string,
  version: string,
  repository: PackageJson["repository"] = serverPackageJson.repository,
) {
  const path = yield* Path.Path;
  const fs = yield* FileSystem.FileSystem;
...
const applyPackageResource = Effect.fn("applyPackageResource")(function* (
  resource: Effect.Success<ReturnType<typeof preparePackageResource>>,
) {
  const fs = yield* FileSystem.FileSystem;
...
const restorePackageResource = Effect.fn("restorePackageResource")(function* (
  resource: Effect.Success<ReturnType<typeof preparePackageResource>>,
) {
  const fs = yield* FileSystem.FileSystem;
...
const assertServerPackageBuild = Effect.fn("assertServerPackageBuild")(function* (
  serverDir: string,
) {
  const path = yield* Path.Path;
  const fs = yield* FileSystem.FileSystem;
...
const packCmd = Command.make(
  "pack",
  {
    outputDirectory: Flag.string("output-directory").pipe(Flag.withDefault("release-publish")),
    verbose: Flag.boolean("verbose").pipe(Flag.withDefault(false)),
  },
  (config) =>
    Effect.gen(function* () {
      ...
      yield* assertServerPackageBuild(serverDir);
      yield* fs.makeDirectory(outputDirectory, { recursive: true });
      yield* Effect.acquireUseRelease(
        preparePackageResource(repoRoot, serverDir, serverPackageJson.version, {
          type: "git",
          url: `https://github.com/${PERSONAL_DISTRIBUTION.repository.owner}/${PERSONAL_DISTRIBUTION.repository.name}`,
          directory: "apps/server",
        }),
        (resource) =>
          Effect.gen(function* () {
            yield* applyPackageResource(resource);
            const args = ["pm", "pack", "--filter", "t3", "--pack-destination", outputDirectory];
        ...
        restorePackageResource,
      );
    }),
).pipe(Command.withDescription("Pack the built server for a GitHub Release."));
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        ...
        (resource) =>
          restorePackageResource(resource).pipe(
            Effect.tap(() =>
              config.verbose ? Effect.log("[cli] Restored original publish assets") : Effect.void,
            ),
          ),
      );
    }),
