...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("resolves the early linux Electron switches", () => {
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "bpdev-code-dev",
      linuxDesktopEntryName: "bpdev-code-dev.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under the Tangent home when T3CODE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.bpdev-code/dev/desktop-settings.json");
