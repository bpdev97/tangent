...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isDevelopment: false,
    displayName: "Tangent",
    linuxDesktopEntryName: "bpdev-code.desktop",
    linuxWmClass: "bpdev-code",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/Tangent.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "Tangent (Nightly)",
      execTarget: '/home/al ice/Apps/Tangent "100%" $HOME\\x.AppImage',
      scheme: "bpdev-code",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=Tangent (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/Tangent \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/bpdev-code;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "bpdev-code",
      desktopEntryPath: "/home/alice/.local/share/applications/bpdev-code.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the bpdev-code:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/bpdev-code.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the bpdev-code:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/Tangent.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/bpdev-code;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "bpdev-code.desktop", "x-scheme-handler/bpdev-code"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      yield* runRegister(recorded, {
        existingEntry: DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
          displayName: "Tangent",
          execTarget: "/home/alice/Applications/Tangent.AppImage",
          scheme: "bpdev-code",
        }),
      });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/bpdev-code.desktop",
        }),
      });
