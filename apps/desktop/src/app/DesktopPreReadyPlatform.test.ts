    ...
    it.effect(
      ...
      () => {
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/bpdev-code.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "bpdev-code.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=Tangent");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/bpdev-code;");
          }),
