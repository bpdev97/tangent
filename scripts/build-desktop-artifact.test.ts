...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // entry advertises the Tangent scheme for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Tangent", schemes: ["bpdev-code", "bpdev-code-dev"] },
      ]);
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.bpdev97.t3code.macos</string>");
  ...
  it("ships the Linux CLI release archive as the WSL runtime", () => {
    ...
    const runtimeArchivePath = "/tmp/tangent-server-1.2.3-linux-x64.tar.gz";
    ...
    assert.equal(wslRuntimeArchiveStem("1.2.3", "x64"), "tangent-server-1.2.3-linux-x64");
  });

  it("parses Windows bsdtar member listings with CRLF line endings", () => {
    assert.deepStrictEqual(
      parseWslRuntimeArchiveMembers(
        "./tangent-server-1.2.3-linux-x64/t3\r\ntangent-server-1.2.3-linux-x64/client/\r\n",
      ),
      ["tangent-server-1.2.3-linux-x64/t3", "tangent-server-1.2.3-linux-x64/client"],
    );
  });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const sourceArchivePath = yield* makeLinuxCliArchiveFixture({
          root,
          stem: "tangent-server-1.2.3-linux-x64",
        });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const error = yield* stageWslRuntimeArchive({
          sourceArchivePath: path.join(root, "tangent-server-1.2.3-linux-x64.tar.gz"),
