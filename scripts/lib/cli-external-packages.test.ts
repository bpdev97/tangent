...
describe("selectCliRuntimeExternalDependencies", () => {
  ...
  it("selects every external root declared by the server", () => {
    assert.deepStrictEqual(
      ...
      ["@ff-labs/fff-node", "heic-decode", "jpeg-js", "libheif-js", "node-pty"],
    );
  });
});
...
//
// Found the hard way: heic-decode's node-gyp-build-optional-packages
...
describe("findEsmImportsOfExternalPackages", () => {
  ...
  it("flags side-effect imports and re-exports too", () => {
    const source = ['import "heic-decode";', 'export { load } from "ffi-rs";'].join("\n");
    assert.deepStrictEqual(findEsmImportsOfExternalPackages(source), ["ffi-rs", "heic-decode"]);
  });
