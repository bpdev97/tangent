...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$(printf '%s\n%s\n' "$version" "${base_url}/personal-v${version}")" ]; then
  ...
  printf '  %sInstalling%s Tangent %s%s%s\n\n' "$muted" "$reset" "$bold" "$version" "$reset" >&2
  step "Downloading..."
  fetch_status=0
  ...
  printf '%s\n%s\n' "$version" "${base_url}/personal-v${version}" > "${staging}/.install-complete"
