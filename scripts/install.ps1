...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}Tangent$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw) -eq "$version`n$baseUrl/personal-v$version`n")) {
  ...
  try {
    ...
    [Console]::Error.WriteLine("  ${muted}Installing$reset Tangent $bold$version$reset`n")
    Step "Downloading..."
    try {
...
if ($interactive) { [Console]::Error.Write("`r$esc[2K") }
[Console]::Error.WriteLine("  ${green}Installed Tangent $version$reset`n")
