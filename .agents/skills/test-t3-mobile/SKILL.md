...
against `~/.bpdev-code/userdata`. The Browser panel is not required for this workflow.
...
`open <app-id> <url>` and all returned target arguments. Resolve the iOS app ID as
`PERSONAL_MOBILE_DISTRIBUTION.iosBundleIdentifier + ".dev"` from
`downstream/mobile-config.ts`; Android uses `com.t3tools.t3code.dev`. Keep the
development scheme from that config (`bpdev-code-dev`).
...
It issues a fresh credential and opens Tangent Dev's existing pairing route
