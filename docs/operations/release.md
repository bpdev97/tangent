...
Tangent follows upstream's standalone CLI format with its personal release identity.
Each `personal-vX.Y.Z` release contains the five platform-specific `tangent-server` archives
and `SHA256SUMS`, alongside the signed macOS desktop assets. The personal workflow builds
and smoke-tests each archive on its native architecture before publishing the complete release.
See [personal distribution](../personal-distribution.md#server-release-artifacts) for the
source, cache, signing, and one-time launcher migration requirements.

Verify successful remote updates, rollback after a failed trial, incompatible launcher
rejection, and desktop-managed/manual update guidance before publishing. The old protocol-2
launcher requires local replacement; do not claim remote migration from 0.1.55.
