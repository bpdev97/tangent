...
that slice, so desktop-only changes never alter the iOS runtime fingerprint. The release slice,
[`downstream/release-config.ts`](../../downstream/release-config.ts), holds the repository and
release tag and archive prefixes for shared release helpers that mobile also imports.
...
- Shared: `packages/shared/src/cliRelease.ts` (repository, tag prefix, archive prefix, read from
  the release slice because mobile imports it) and
...
- Never import `downstream/config.ts` from mobile or Expo code; use `downstream/mobile-config.ts`
  or, for shared release helpers, `downstream/release-config.ts`.
