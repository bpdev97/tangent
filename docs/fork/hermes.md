...
- A gateway below the minimum contract is reported as incompatible, with the update action, instead
  of failing mid-turn.
- The compatibility advisory comes from the bundled `hermes` policy: versions from the oldest one
  Tangent has tested are supported, and older ones are `unknown` rather than `unsupported`, because
  the contract check above already reports gateways Tangent cannot drive. Upstream's remote manifest
  has no Hermes entry, so the bundled policy always applies.

Compatibility baseline: Hermes Agent 0.21.5 (`v2026.9.24`), gateway contract 8. The minimum is
contract 7; contract 8 only changed connector RPCs, which Tangent does not call. Because
Tangent can update Hermes itself, it does not keep workarounds for older gateways. The values live
in `downstream/fork.json` under `hermes`.
...
- `apps/server/src/provider/model-manifest.json`: the `hermes` entry under `compatibility`, next
  to Pi's. JSON has no comments, so this hook carries no marker.
...
- `model-manifest.json`: take upstream's version and re-add the `hermes` compatibility entry. Raise
  its supported floor only when a baseline move drops support for older gateways.
