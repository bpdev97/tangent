...
## Tangent release source

Tangent installs the exact `tangent-server-X.Y.Z.tgz` asset from its `personal-vX.Y.Z` GitHub Release, verifies its SHA-256 sidecar, and retains a separate state root and service identity. A matching upstream npm version is not interchangeable. See [personal distribution](../personal-distribution.md).
