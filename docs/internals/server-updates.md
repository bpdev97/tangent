...
> For maintainers. Using Tangent? See [docs/user](../user/).
...
| ----------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `boot-service`    | Linux server running under the Tangent-managed `tangent.service` systemd user unit            | Call the update RPC; the service unit is replaced and restarted.      |
| `respawn`         | Packaged Tangent CLI running in the foreground on macOS or Linux                              | Call the update RPC; the process hands off to a detached replacement. |
...
| absent            | Older server, development checkout, Windows foreground process, or an unrecognized supervisor | Offer the exact Tangent GitHub Release relaunch command.              |
...
flowchart TD
    ...
    F --> H[Download and install exact Tangent release]
    G --> H
    ...
    K -->|boot-service| L[Rewrite and restart tangent.service]
...
payload accepts only an exact release version, including an exact prerelease version; floating tags
such as `latest` and `nightly` are rejected. The unary `server.updateServer` method remains available so a
new client can still repair skew with an older server.

For version `X.Y.Z`, the downloader resolves tag `personal-vX.Y.Z`, archive
`tangent-server-X.Y.Z.tgz`, and checksum `tangent-server-X.Y.Z.tgz.sha256` from
`bpdev97/tangent`. It verifies the archive SHA-256 before installing it under
`<baseDir>/runtime/versions/<version>`. The completion sentinel records the release identity and
artifact hash. Boot-service setup and self-update share the same process-wide installation lock, so
...
the case where a completed package install produced an unusable tree, so retrying the same version must
...
reconcile the `tangent.service` unit through `BootService`.
...
For `boot-service`, the server atomically rewrites the Tangent-managed user unit to point at the verified
...
The exact client version and checksum-verified server archive must exist in the matching personal
GitHub Release before a client carrying that version is published. See
[Release Checklist](../operations/release.md#server-self-update-release-invariant).
