...
- `versions/<version>`, immutable exact-version installs from the verified personal release asset.
...
1. The active server downloads the exact `tangent-server-<target>.tgz` and checksum from the
   matching `bpdev97/tangent` GitHub Release, verifies SHA-256, and installs it into a unique
   staging directory.
...
Each completed runtime records both its version and release-source identity. This prevents a
same-version upstream npm package from satisfying Tangent's install check. The stable launcher
reads the version from the first sentinel line so it retains upstream's rollback state machine
without owning distribution policy.
