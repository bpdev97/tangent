...
On Linux and macOS, Tangent can run as a service for your user so you do not need
...
Linux needs systemd user services. Setup enables lingering so Tangent starts at
...
with sudo; running Tangent as root creates a separate installation and Connect
...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status tangent.service`, then use the repair command printed by Tangent.                    |
