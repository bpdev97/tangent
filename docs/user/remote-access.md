...
Use this when you want the desktop app to start or reuse Tangent on another machine over SSH.
...
When the Tangent web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Tangent may
...
Hosted pairing does not proxy traffic through Tangent. The browser still connects directly to the backend URL in the pairing link.
