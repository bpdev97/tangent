...
Connect a phone, browser, or another desktop app to Tangent running on a different
...
```bash
npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz connect
```

Follow the sign-in instructions. Setup offers a
[background service](./background-service.md); if you decline it, start the
server with `npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz serve`. Saving your sign-in alone does not make the machine
reachable.
...
```bash
npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz serve --host <private-ip>
```

If a server is already running, generate a fresh link without restarting it:

```bash
npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz pair
```
...
```bash
npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz serve --tailscale-serve
```

For an already-running server:

```bash
npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz pair --tailscale
```
...
`--tailscale-serve-port`. See `npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz pair --help` for other pairing options.
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. Tangent starts
...
the connection stops a server that Tangent launched; a server that was already
...
management is available through `npx --yes https://github.com/bpdev97/tangent/releases/download/personal-v<version>/tangent-server-<version>.tgz auth --help`.
...
| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart Tangent on the host.                                                                         |
...
| Expired or invalid link proof                             | Check the host's date and time, update Tangent, then restart it.                                                                            |
...
`systemctl --user restart tangent.service` for the background service. For a
...
devices. For server version warnings, follow [Updating Tangent](./updating.md).
