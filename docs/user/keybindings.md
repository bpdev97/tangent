...
`~/.bpdev-code/userdata/keybindings.json` by default. You can edit this file directly.
...
In the command palette, Control-N and Control-P move to the next and previous result, using the same highlighting and scrolling as the arrow keys. Tangent also supports the desktop Quick Chat action `bpdev-code-action://quick-chat`.
