...
official app's state or update source. Each personal release includes
platform-specific `tangent-server-X.Y.Z-<platform>` standalone archives and `SHA256SUMS`; local
update, SSH launch, service installation, and pinned runtime resolution all use those verified
GitHub Release artifacts. The archive and launcher layout follow upstream.
...
| `FORK-IMAGE-001`     | HEIC/HEIF normalization, mobile image shrinking, and camera capture                          | Active               | [Image normalization](docs/fork/image-normalization.md)               | `vp test packages/contracts/src/orchestration.test.ts apps/server/src/imageNormalization.test.ts apps/server/src/orchestration/Normalizer.test.ts apps/mobile/src/lib/composerImages.test.ts apps/mobile/src/lib/prepareComposerImage.test.ts apps/mobile/src/lib/composerFiles.test.ts apps/mobile/src/features/sharing/incoming-share-model.test.ts`                                                                                                                                                      |
...
| `FORK-PREVIEW-001`   | Suspend inactive browser webviews for server-settled threads                                 | Active               | [Settled preview suspension](docs/fork/settled-preview-suspension.md) | `vp test apps/web/src/browser/browserSessionSuspension.test.ts apps/web/src/browser/desktopTabLifetime.test.ts`                                                                                                                                                                                                                                                                                                                                                                                             |
| `FORK-SHORTCUTS-001` | iOS App Shortcuts for new managed chats and foreground dictation                             | Active               | [iOS App Shortcuts](docs/fork/ios-app-shortcuts.md)                   | `vp test apps/mobile/plugins/withIosAppShortcuts.test.ts apps/mobile/src/features/shortcuts/iosAppShortcuts.test.ts packages/client-runtime/src/voice-input/controller.test.ts` plus an iOS 26 simulator shortcut pass and supported-device dictation pass                                                                                                                                                                                                                                                  |
...
## Restored to upstream on 2026-09-11

`FORK-LINEAR-001` is deprecated. PR badges, destinations, and browser presentation follow upstream.
Linear resolution, settings, RPCs, and dedicated preview behavior have been removed at the maintainer’s
request. Stored Linear credentials are left untouched and unused. Reintroducing the integration
requires a separately documented Tangent reason.
...
Shared touchpoints include the server upload normalizer, mobile image inputs and attachment menus,
mobile app config/package, the opt-in native menu-order patch, and workspace lockfile. Mobile input preparation is owned by
`apps/mobile/src/lib/prepareComposerImage.ts` and its focused test.

Invariants:

- Mobile prepares library, camera, clipboard, and shared images before the existing 10 MiB upload
  contract. Keep smaller images unchanged, preserve PNG transparency, and never silently flatten
  oversized GIF/WebP images. Bound compression attempts and release temporary native resources.
- Keep Camera immediately below Photo Library in new-thread, existing-thread, question, and review composers,
  including servers without generic file uploads. Cancellation/denial preserves the draft.
- Changes to mobile native dependencies or camera configuration require an iOS native release.
...
- A server-settled thread keeps its webviews while it is the active route and suspends them
...
- `apps/mobile/plugins/withIosAppShortcuts.cjs` and its focused test
- `apps/mobile/modules/t3-native-controls/ios/T3AppShortcutsModule.swift`
- `apps/mobile/src/features/shortcuts/iosAppShortcuts.ts` and its focused test
- `apps/mobile/src/native/appShortcutActions.ios.ts`
- `apps/mobile/src/native/appShortcutActions.ts`
- `docs/fork/ios-app-shortcuts.md`

Shared upstream touchpoints are the mobile app config, native-controls module, root shortcut hook,
generic-chat project resolver, and new-task draft route/composer.

Invariants:

- Expose only fixed New Chat and Dictate New Chat actions; never turn native shortcut payloads into
  arbitrary navigation or command execution.
- Resolve the managed chat and preferred host in shared client logic after hydration. Do not persist
  environment-local IDs in App Intent metadata.
- Keep pending delivery durable across cold start and clear by matching request ID only after
  navigation dispatches.
- Start dictation once, only after the generic-chat draft is ready, through the existing foreground
  voice controller. Never auto-submit shortcut dictation.
- Dictate New Chat remains iOS 26-or-later while local Apple transcription has that minimum.
- Ship App Intent changes in a native iOS build, never as an OTA-only release.
