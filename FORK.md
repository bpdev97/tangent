...
- Ticket destinations follow the server-scoped `ticketOpenBehavior`: either the existing Tangent
  side panel or a validated Linear desktop-app deep link. Review remains in the side panel.
...
- In Tangent mode, Review and each ticket own a distinct browser tab. Reopening a destination
