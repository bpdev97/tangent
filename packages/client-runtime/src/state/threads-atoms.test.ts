...
describe("createEnvironmentThreadStateAtoms", () => {
  ...
  it("allows memory-constrained clients to shorten inactive retention", () => {
    const runtime = Atom.runtime(Layer.empty) as unknown as Atom.AtomRuntime<
      EnvironmentRegistry | EnvironmentCacheStore | ThreadSnapshotLoader,
      never
    >;
    const threads = createEnvironmentThreadStateAtoms(runtime, { idleTtlMs: 1_000 });
    const atom = threads.stateAtom(EnvironmentId.make("environment-1"), ThreadId.make("thread-1"));

    expect(atom.idleTTL).toBe(1_000);
  });
});
