...
export function createEnvironmentThreadDetailAtoms<E>(
  ...
  const threadStateValueAtomFamily = Atom.family((key: string) => {
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-state-value:${key}`));
  });
  ...
  const threadDetailAtomFamily = Atom.family((key: string) => {
    ...
    return Atom.make((get) => {
      ...
      return previousValue;
    }).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-detail:${key}`));
  });
  ...
  const threadMessagesAtomFamily = Atom.family((key: string) =>
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-messages:${key}`)),
  );
  ...
  const threadActivitiesAtomFamily = Atom.family((key: string) =>
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-activities:${key}`)),
  );
  ...
  const threadProposedPlansAtomFamily = Atom.family((key: string) =>
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-proposed-plans:${key}`)),
  );
  ...
  const threadCheckpointsAtomFamily = Atom.family((key: string) =>
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-checkpoints:${key}`)),
  );
  ...
  const threadSessionAtomFamily = Atom.family((key: string) =>
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-session:${key}`)),
  );
  ...
  const threadLatestTurnAtomFamily = Atom.family((key: string) =>
    ...
    ).pipe(Atom.setIdleTTL(idleTtlMs), Atom.withLabel(`environment-thread-latest-turn:${key}`)),
  );
