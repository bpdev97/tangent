...
const CLI_RELEASE_REPOSITORY = `${PERSONAL_DISTRIBUTION.repository.owner}/${PERSONAL_DISTRIBUTION.repository.name}`;
...
export function cliArchiveFileName(version: string, platformKey: CliArchivePlatformKey): string {
  return `${PERSONAL_DISTRIBUTION.serverRelease.artifactNamePrefix}-${version}-${platformKey}.${platformKey.startsWith("win32") ? "zip" : "tar.gz"}`;
}
...
export function cliReleaseDownloadBaseUrl(
  ...
  return `${(baseUrl?.trim() || CLI_RELEASE_DEFAULT_BASE_URL).replace(/\/+$/, "")}/${PERSONAL_DISTRIBUTION.serverRelease.tagPrefix}${version}`;
}
...
export function newestCliReleaseVersion(
  ...
  for (const release of releases) {
    ...
    const version = /^(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)$/.exec(
      release.tag_name.startsWith(PERSONAL_DISTRIBUTION.serverRelease.tagPrefix)
        ? release.tag_name.slice(PERSONAL_DISTRIBUTION.serverRelease.tagPrefix.length)
        : "",
    )?.[1];
...
/** Cache identity includes the release source, even when two distributions share a version. */
export function cliRuntimeInstallSentinel(version: string, baseUrl?: string): string {
  return `${version}\n${cliReleaseDownloadBaseUrl(version, baseUrl)}\n`;
}
