...
describe("cliRelease", () => {
  it("names archives by version and platform, zip only on Windows", () => {
    expect(cliArchiveFileName("1.2.3-preview.20260911.4", "linux-x64")).toBe(
      "tangent-server-1.2.3-preview.20260911.4-linux-x64.tar.gz",
    );
    expect(cliArchiveFileName("1.2.3", "win32-x64")).toBe("tangent-server-1.2.3-win32-x64.zip");
  });
  ...
  it("resolves download URLs under the tagged release, honoring a mirror", () => {
    expect(cliReleaseDownloadBaseUrl("1.2.3")).toBe(
      "https://github.com/bpdev97/tangent/releases/download/personal-v1.2.3",
    );
    expect(cliReleaseDownloadBaseUrl("1.2.3", "https://mirror.example/t3/")).toBe(
      "https://mirror.example/t3/personal-v1.2.3",
    );
  });
  ...
  it("parses sha256sum output including binary-mode markers", () => {
    const checksums = parseChecksums(
      [
        `${"a".repeat(64)}  tangent-server-1.2.3-linux-x64.tar.gz`,
        `${"B".repeat(64)} *tangent-server-1.2.3-win32-x64.zip`,
        "not a checksum line",
        "",
      ].join("\n"),
    );
    expect(checksums.get("tangent-server-1.2.3-linux-x64.tar.gz")).toBe("a".repeat(64));
    expect(checksums.get("tangent-server-1.2.3-win32-x64.zip")).toBe("b".repeat(64));
  ...
  it("picks the newest non-draft release on the requested channel", () => {
    const releases = [
      ...
      { tag_name: "personal-v1.2.3" },
      { tag_name: "personal-v1.2.3-nightly.20260911.2" },
    ];
    ...
    expect(newestCliReleaseVersion([{ tag_name: "personal-v1.2.3" }], "preview")).toBeUndefined();
  });

  it("pages through the release index at the largest page GitHub allows", () => {
    expect(cliReleaseIndexPageUrl(1)).toBe(
      "https://api.github.com/repos/bpdev97/tangent/releases?per_page=100&page=1",
    );
