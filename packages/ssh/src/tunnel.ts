...
T3_RUNTIME_DIR="$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}/runtime/versions/$T3_ARCHIVE_VERSION"
t3_runtime_ready() {
  [ -x "$T3_RUNTIME_DIR/t3" ] && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$(printf '%s\\n%s\\n' "$T3_ARCHIVE_VERSION" "$T3_RELEASE_BASE_URL")" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}/runtime/versions"
  ...
  # under it.
  T3_LOCK="$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_ARCHIVE="${PERSONAL_DISTRIBUTION.serverRelease.artifactNamePrefix}-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"
  T3_STAGING="$(mktemp -d "$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}/runtime/versions/.staging-XXXXXX")"
  ...
  t3_fetch "$T3_RELEASE_BASE_URL/SHA256SUMS" "$T3_STAGING/SHA256SUMS" @@T3_ARCHIVE_CHECKSUMS_SECONDS@@
  t3_fetch "$T3_RELEASE_BASE_URL/$T3_ARCHIVE" "$T3_STAGING/$T3_ARCHIVE" @@T3_ARCHIVE_DOWNLOAD_SECONDS@@
  ...
  printf '%s\\n%s\\n' "$T3_ARCHIVE_VERSION" "$T3_RELEASE_BASE_URL" > "$T3_STAGING/.install-complete"
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}"
...
STATE_DIR="$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/${PERSONAL_DISTRIBUTION.macos.stateHomeDirectoryName}"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
