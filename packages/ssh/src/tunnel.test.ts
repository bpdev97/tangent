...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/bpdev97/tangent/releases/download/personal-v1.2.3-preview.20260911.4'",
    );
    assert.include(
      script,
    ...
    assert.include(
      launch,
      "T3_RELEASE_BASE_URL='https://mirror.example/t3/personal-v1.2.3-preview.20260911.4'",
    );
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(
          ...
          `${archiveVersion}\nfile://${root}/mirror/personal-v${archiveVersion}`,
        );
