...
## Tangent fork requirements

Tangent is a personal fork of T3 Code. Before rebasing from upstream or changing a fork-owned
...
Claude Code, Cursor, Grok, OpenCode, Antigravity, and the fork-owned Hermes integration) and serves web, desktop,
and mobile clients.
...
- **Providers.** Codex, Claude, Cursor, Grok, OpenCode, Antigravity, and Hermes each have an adapter.
...
- User-visible frontend changes get one integrated pass in a real client: `test-t3-app` for web,
  `test-t3-mobile` for mobile. The primary agent does this once after integrating. Subagents do not
  launch their own dev servers.
